...
```python
from oceens.billing import total_due              # fine: the package's public surface
from oceens.billing.quote import estimate         # fine: a further public entry point
from oceens.billing._internal._totals import total_due   # fails `tach check`
```
...
the shape applies to every package under `src/oceens/`, whichever they are.
...
The packages here are `core`, `models`, `routers` and `services`, plus the
loose modules beside them (`main`, `summaries_generator_daemon`, …). They do
not have this shape yet: their `__init__.py` files are mostly empty, and
callers import the public modules directly (`oceens.core.security`,
`oceens.services.llm_client`). The check already holds them to the rule —
nothing private crosses a package boundary, and no two packages import each
other. Deciding what each package should expose, and hide, is design work
tracked in #93.
...
There are no tests yet (#85). When a `tests/` directory is added at the repo
root, add `"tests"` to `source_roots` in `tach.toml`: tests are code outside
every package, so the same rule then forces them through the public surfaces
too. (tach fails outright on a source root that does not exist, which is why it
is not listed today.)
